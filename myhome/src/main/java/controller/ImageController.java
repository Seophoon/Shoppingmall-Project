...
public class ImageController {
	...
	public ModelAndView replyForm(Integer id, Integer parentid, Integer groupid) {
		...
		bbs.setImage_name(null);//이미지 이름을 지운다.`												
	...
	public ModelAndView updateDo(ImageBBS imageBBS, HttpSession session,
		...
		ImageBBS old = this.imageDao.getImageDetail(Integer.parseInt(multipart.getParameter("w_id")));//글번호로 조회
		ModelAndView mav = new ModelAndView("home/template");
		if(old.getPassword().equals(multipart.getParameter("password"))) {//암호가 일치하는 경우
			...
			if(image_name == null) {//파일이름이 없는 경우, 즉 이미지 변경을 안한 경우
				imageBBS.setImage_name(old.getImage_name());//기존의 이미지 이름을 설정
			}else {//파일 이름이 있는 경우, 즉 이미지 변경을 하는 경우
	...
	public ModelAndView write(ImageBBS imageBBS, HttpSession session,
		...
		imageBBS.setId(id);//계정설정
		imageBBS.setPassword(multipart.getParameter("password"));//암호설정
