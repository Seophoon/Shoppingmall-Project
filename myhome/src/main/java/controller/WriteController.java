...
public class WriteController {
	...
	public ModelAndView bbsModify(Integer SEQNO, String CONTENT, String BTN, String TITLE) {
	...
	public ModelAndView bbsDetail(Integer SEQNO, HttpSession session) {
		...
		if(id == null || ! id.equals(bbs.getId())) {//작성자와 계정이 다른 경우(로그인을 한 경우, 로그인을 안한 경우
	...
	public ModelAndView bbsRead(Integer PAGE_NO) {
		...
		mav.addObject("STARTROW", startRow+1);//현재페이지의 시작글번호
		if((endRow-1) > totalCount) endRow = totalCount+1;
		mav.addObject("ENDROW", endRow-1);//현재 페이지의 마지막글번호
